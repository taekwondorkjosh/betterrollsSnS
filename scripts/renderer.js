...
function renderModuleTemplate(path, props) {
	return renderTemplate(`modules/betterrollsSnS/templates/${path}`, props);
}
...
export class Renderer {
	...
	static async renderDamage(properties, settings) {
		...
		if (!titleString && CONFIG.SNS.healingTypes[damageType]) {
			// Show "Healing" prefix only if it's not inherently a heal action
			titleString = "";
		} else if (!titleString && CONFIG.SNS.damageTypes[damageType]) {
		...
		if (isVersatile) {
			damageStringParts.push("(" + CONFIG.SNS.weaponProperties.ver + ")");
		}
	...
	static async renderSaveButton(properties) {
		const abilityLabel = CONFIG.SNS.abilities[properties.ability];
