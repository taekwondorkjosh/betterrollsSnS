...
import { SNS } from "../../../../systems/sns/module/config.js";
...
export class ActorUtils {
	...
	static isHalfling(actor) {
		return getProperty(actor, "data.flags.sns.halflingLucky");
	}
	...
	static hasReliableTalent(actor) {
		return getProperty(actor, "data.flags.sns.reliableTalent");
	}
	...
	static hasElvenAccuracy(actor) {
		return getProperty(actor, "data.flags.sns.elvenAccuracy");
	}
	...
	static getMeleeExtraCritDice(actor) {
		return actor?.getFlag("sns", "meleeCriticalDamageDice") ?? 0;
	}
	...
	static getCritThreshold(actor, itemType) {
		...
		const actorFlags = actor.data.flags.sns || {};
...
export class ItemUtils {
	static getActivationData(item) {
		...
		if (activation?.type && activation?.type !== "none") {
			return `${activationCost} ${sns.abilityActivationTypes[activation.type]}`.trim();
		}
	...
	static getDuration(item) {
		...
		return `${duration.value ? duration.value : ""} ${sns.timePeriods[duration.units]}`.trim()
	}
	...
	static getRange(item) {
		...
		const rangeUnit = range.units ? sns.distanceUnits[range.units] : "";
	...
	static getTarget(item) {
		...
		const targetDistance = target.units && target?.units !== "none" ? ` (${target.value} ${sns.distanceUnits[target.units]})` : "";
		return i18n("Target: ") + sns.targetTypes[target.type] + targetDistance;
	}
	...
	static placeTemplate(item) {
		if (item?.hasAreaTarget) {
			const template = game.sns.canvas.AbilityTemplate.fromItem(item);
	...
	static getPropertyList(item) {
		...
		switch(item.data.type) {
			case "weapon":
				properties = [
					sns.weaponTypes[data.weaponType],
					range,
					target,
				...
				for (const prop in data.properties) {
					if (data.properties[prop] === true) {
						properties.push(sns.weaponProperties[prop]);
					}
				}
				break;
			case "spell":
			...
			case "equipment":
				properties = [
					sns.equipmentTypes[data.armor.type],
			...
			case "tool":
				properties = [
					sns.proficiencyLevels[data.proficient],
					data.ability ? sns.abilities[data.ability] : null,
					data.weight ? data.weight + " lbs." : null,
				];
				break;
			case "loot":
