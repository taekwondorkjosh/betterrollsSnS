import { isAttack, isSave } from "./betterrollsSnS.js";
...
export async function addBetterRollsContent(app, protoHtml) {
	...
	const altSecondaryEnabled = game.settings.get("betterrollsSnS", "altSecondaryEnabled");
	...
	const betterRollsTemplate = await renderTemplate("modules/betterrollsSnS/templates/red-item-options.html", {
		SNS: CONFIG.SNS,
		item,
		canConsume,
		hasQuantity,
	...
	if (game.settings.get("betterrollsSnS", "damageContextPlacement") !== "0") {
		...
		const placeholder = game.settings.get("betterrollsSnS", "contextReplacesDamage") ? "br5e.settings.label" : "br5e.settings.context";
